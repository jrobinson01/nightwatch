...
module.exports = function(Nightwatch) {
  ...
  Actions.element = function(using, value, callback) {
    ...
    if (using == 'css selector' &&  Nightwatch.options.desiredCapabilities.browserName == 'chrome') {
      //
      //console.log('element using css selector:', using, value);
      var cb = function(result){
        //console.log('cb wrapper:', result.status, 'value', value);
        if (result.status !== 0) {
          //we haven't tried deep yet.
          if (value.indexOf('/deep/') === -1) {
            if (value.indexOf('html') !== 0){
              //try 'html /deep/ '
              value = 'html /deep/ ' + value;
              //console.log('trying with /deep/', value);
              return element(using, value, cb);
            } else {
              //we tried 'html /deep/ ' , no luck.
              //TODO: we need to handle multiple spaces in a selector,
              //and try every combination of ' /deep/ '. Refer to Casey's
              //code in BW...
              value = value.replace('html /deep/ ','');
              if (value.indexOf(' ')> -1) {
                //
                value = value.split(' ').join(' /deep/ ');
                return element(using, value, cb);
              } else {
        ...
        //console.log('running callback:', result.status);
        return callback(result);
      };
      //console.log('using recursive callback:', value);
      return element(using, value, cb);
    }
  ...
  Actions.elements = function(using, value, callback) {
    ...
    if (using == 'css selector' &&  Nightwatch.options.desiredCapabilities.browserName == 'chrome') {
      //
      //console.log('ELELEMTS using css selector:', using, value);
      var cb = function(result){
        //console.log('cb wrapper:', result.status, 'value', value, result.value);
        if (result.status == 0 && result.value.length == 0) {
          ...
          if (value.indexOf('/deep/') === -1) {
            if (value.indexOf('html') !== 0){
              ...
              value = 'html /deep/ ' + value;
              //console.log('trying with /deep/', value);
              return elements(using, value, cb);
            } else {
              //we tried 'html /deep/ ' , no luck.
              //TODO: we need to handle multiple spaces in a selector,
              //and try every combination of ' /deep/ '. Refer to Casey's
