...
module.exports = function(Nightwatch) {
  ...
  Actions.element = function(using, value, callback) {
    ...
    if(using == 'css selector' && !Nightwatch.SUPPORTS_SHADOW_DOM){
      value = value.split('html /deep/ ').join('').split(' /deep/ ').join(' ');
    }
  ...
  */
  ...
  Actions.url = function(url, callback) {
    //check shadow dom support first!
    this.execute('return document.head.createShadowRoot != undefined;',[], function(response){
      console.log('supports shadow dom?', response.value, Nightwatch.SUPPORTS_SHADOW_DOM);
      Nightwatch.SUPPORTS_SHADOW_DOM = response.value;
